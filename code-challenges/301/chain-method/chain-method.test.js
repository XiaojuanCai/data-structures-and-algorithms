...
const count = (target, input) => {
  var countElements = (arr, searchElement) => {
      return arr.reduce((acc,v,i) => {
          if (v === searchElement) acc++;
          return acc;
      }, 0);
  };
  return input.reduce((acc,v,i) => {
      acc += countElements(v,target);
      return acc;
  },0);
};
...
const totalSum = (input) => {
  return input.reduce((sum,arr,i) => {
    sum += arr.reduce((acc,v,i) => {
        acc += v;
        return acc;
    },0);
    return sum;
...
// console.log(totalSum([[1, 3, 5, 7, 9], [5, 5, 5], [1, 2, 3]]));
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((arr,i) => {
      var removeFive = arr.filter((v,j) => {
          return (typeof v) === 'number' && v % 5 === 0;
      });
      return removeFive.map((v,j) => {
          return 2 ** v;
      });
  });
};
...
let findMaleAndFemale = (data) => {
  var humans = data.filter((v,i) => {
      return (v.gender === 'male' || v.gender === 'female');
  });
  var humansNames = humans.map((v,i) => {
      return v.name;
  });
  return humansNames.join(' and ');
};
