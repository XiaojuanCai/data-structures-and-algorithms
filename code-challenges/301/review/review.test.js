...
const findLukeMass = (arr) => {
    var lukeMass = null;
    arr.forEach((v,i) => {
        if (v.name.includes('Luke ')) lukeMass = parseInt(v.mass);
    });
    return lukeMass;
};
let biggerThanLuke = (arr) => {
  let lukeMass = findLukeMass(arr);
  if (lukeMass === null) return '';
  let bigger = arr.filter((v,i) => {
    return parseInt(v.mass) > lukeMass;
  });
  let biggerNames = bigger.map((v,i) => {
      return v.name;
  });
...
// console.log(isSecure('https://www'));
...
const ticTacToeWinHelper = (row1, col1, row2, col2, row3, col3, board) => {
    return board[row1][col1] === 'X'
    && board[row2][col2] === 'X'
    && board[row3][col3] === 'X';
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (const r in [0,1,2]){
    if (ticTacToeWinHelper(r,0,r,1,r,2, board)) return true;
  }
  // every col
  for (const c in [0,1,2]){
    if (ticTacToeWinHelper(0,c,1,c,2,c, board)) return true;
  }
  // diagonal 
  if (ticTacToeWinHelper(0,0,1,1,2,2, board)) return true;
  if (ticTacToeWinHelper(0,2,1,1,2,0, board)) return true;
...
// console.log(detectTicTacToeWin(board));
...
describe('Testing challenge 5', () => {
  test('It should return true if there are three in a row', () => {
    ...
    expect(detectTicTacToeWin([['O', '', 'X'], ['X', 'X', 'X'], ['X', '', 'O']])).toStrictEqual(true);
  });
