...
// console.log(standardizePhoneNumbers(['(123) 456-7890', '(123) 456-7890']));
...
const onlyOddChars = (str) => {
  let oddChars = str.split('').reduce((acc,v,i) => {
      if (i%2 !== 0) acc.push(v);
      return acc;
  },[]);
...
const allHappy = (arr) => {
  return arr.reduce((acc,v,i) => {
      if (!v.includes(':)')) acc = false;
      return acc;
  }, true);
};

// console.log(allHappy([':)add',':)','adkdkkdk:)))))))']));
...
// console.log(findAnything([':)add',':)','adkdkkdk:)))))))'],'add'));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 7
...
const findEvery = (arr, target) => {
  return arr.reduce((acc,v,i) => {
      if (!v.includes(target)) acc = false;
      return acc;
  }, true);
};

// console.log(findEvery([':)add',':-)','adkdkkdk:)))))))'], ':)'));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 8
...
const unenrollBrook = (arr) => {
  return arr.map((enrolls,i) => {
     var newEnrolls = [];
     enrolls.forEach((v,j) => {
         if (!v.toLowerCase().includes('brook')) newEnrolls.push(v);
     }); 
     return newEnrolls;
  });
};

// var arr = [['Brook Testing', 'Actual Person'], ['Human Person', 'Brook again', 'still Brook']];
// console.log(unenrollBrook(arr));
...
const sortByDay = (arr) => {
  let newArr = [[],[],[],[],[],[],[]];
  arr.forEach((str,i) =>{
    for (const j in daysOfWeek){
        if (str.includes(daysOfWeek[j])) newArr[j].push(str);
    }
  });
  return newArr;
};

// let arr = ['Tuesday', 'Monday', 'Wednesday and Thursday', 'Tuesday 2', 'Thursday'];
// console.log(sortByDay(arr));
/* ------------------------------------------------------------------------------------------------
...
// console.log(characterByIndex(['abcd', 'efgh', 'ijkl', 'mno']));
