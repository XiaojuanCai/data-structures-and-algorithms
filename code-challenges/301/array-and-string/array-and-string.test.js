...
// const integers = [1,2,4, 3, 6, 5, 8, 9, 10];
// removeEvenValues(integers);
...
const extractVowels = (str) => {
    ...
    for (let i = 0; i < str.length; i++){
        if (vowels.indexOf(str[i]) < 0){
            newNonVowelStr.push(str[i]);
        }
        else {
            newVowelStr.push(str[i]);
        }
    }
    return [newNonVowelStr.join(''), newVowelStr.sort().join('')];
};

// console.log(extractVowels('gregory'));
/* ------------------------------------------------------------------------------------------------
TESTS
