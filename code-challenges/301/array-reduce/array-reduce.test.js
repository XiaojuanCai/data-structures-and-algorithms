...
// console.log(countNumberOfElements([1,"hi",2,3,4,5]));
...
const reversedString = (str) => {
  var revArr = str.split('').reduce((acc,v,i) => {
      acc.unshift(v);
      return acc;
  },[]);
  return revArr.join('');
...
// console.log(reversedString("abcddddddeeeff"));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
// console.log(countPrimeNumbers([1,2,2,2,4,5,78,6]));
...
const extractChildren = (arr) => {
  var nameWithA = arr.filter((v,i) => {
      return /a+/.test(v.name);
  });
  return nameWithA.reduce((acc,v,i) => {
      if (v.hasOwnProperty('children')){
          acc = acc.concat(v.children);
      }
      return acc;
  }, []);
};
